...
import { CropInfoComponent } from './cropInfo.component';
...
export class IndexEditor implements OnInit {
    ...
    @ViewChild('cropInfo') cropInfoComponent: CropInfoComponent;
    ...
    ngOnInit() {
        ...
        // 根据用户显示默认区域
        if (this.areaServ.level == 3) {
            this.areaServ.getByFilter(Number(this.areaServ.areaCode), this.areaServ.level).then(area => {
                this.stdServ.getIdsByArea(area[0].cCode, area[0].vLevel).then(statIds => {
                    area[0].stationIds = statIds;
                    this.onAreaChange(area[0]);
                });
            });
        }
    ...
    addCrop(item: CropItem): void {
        ...
        this.tabs.push({
            ...
            removable: true,
            active: true,
            customClass: 'tab-remove-level' + this.areaServ.level,
            data: selCropList
        });
    }
    ...
    addDate(): void {
        ...
        if (this.endDate < this.startDate) {
            this.yzNgxToastyService.warning('结束日期不可小于开始日期', "", 3000);
            return;
        }
        ...
        this.cropInfoComponent.onSelect(cropInfo);
    }
    ...
    addDist(): void {
        if (!this.selCropItem || !this.selCropItem.tag)
            return;
        if (!this.selDistItem) {
            this.yzNgxToastyService.warning('请选择灾害', "", 3000); return;
        }
        // 已存在
        if (this.selCropItem.tag) {
            let isFind = false;
    ...
    showDistInfo(title: string, info: IndexInfo): void {
        ...
        if (info.type === '2' && info.values && info.values.length === 2) {
            this.modalRef.content.jiance = info.values[0];
            this.modalRef.content.yubao = info.values[1];
        }
    }

    showAreaInfo(title: string): void {
        if (this.areaServ.level == 3) return;
    ...
    public onSubmit(values: Object): void {
        if (this.cropInfoMap.size <= 0) {
            this.yzNgxToastyService.warning('至少添加一种作物', "", 3000);
            return;
        }
        if (!this.selCropItem.tag) {
            this.yzNgxToastyService.warning('请选择日期', "", 3000);
            return;
        }
        if (this.selCropItem.tag.varidevs.length <= 0) {
            this.yzNgxToastyService.warning('至少添加一种发育期', "", 3000);
            return;
        ...
        expEleMaps.forEach((value, infokey) => {
            ...
            if (value != null) {
                value.forEach(di => {
                    ...
                    distInfo.desc = this.distList.find(dl => dl.code == di.cDistcode) == undefined ? "" : this.distList.find(dl => dl.code == di.cDistcode).desc;
